...
emitter_points = [;];
num_points_on_seg = [];
for i=1:length(emitter_vertices)-2
    ...
    num_points_on_seg = [num_points_on_seg; length(points)];
end
emitter_points = [emitter_points; -x_gap/2-h-l, 0];

num_segs = 0;
emitter_seg_array = [];
for i=1:length(emitter_points)-1
    if emitter_points(i, 1) ~= emitter_points(i+1, 1) || emitter_points(i, 2) ~= emitter_points(i+1, 2)
        seg = int32(1);
...
collector_vertices = [x_gap/2, 0;x_gap/2+h, w/2;x_gap/2+h+l, w/2;x_gap/2+h+l, 0;x_gap/2, 0];
collector_points = [;];
