function [emitter_points, emitter_seg_array, collector_points, collector_seg_array, gate_points, gate_seg_array] = set_up_geometry(emitter_vertices, collector_vertices, gate_vertices, Emitter, Collector, Gate)
...
emitter_points = [emitter_points; mitter_vertices(end, :)];
em_indecies_of_corners = [em_indecies_of_corners; corner_ind];
...
gate_points_pr = gate_points(4:end-2,:);
...
[iErr] = View_Set2DLimits(Lorentz, -3*100, -3*100, 3*100, 3*100, iErr);
...
% pert_ind = 26;
% emitter_points(pert_ind, :) = emitter_points(pert_ind, :) + [0, 5];
% [iErr] = Object_RemoveSegment(Lorentz, Emitter, emitter_seg_array(pert_ind -1), iErr);
% [iErr] = Object_RemoveSegment(Lorentz, Emitter, emitter_seg_array(pert_ind), iErr);
% [emitter_seg_array(pert_ind-1), iErr] = Geometry2D_CreateLine(Lorentz, emitter_points(pert_ind -1,1), emitter_points(pert_ind -1,2),emitter_points(pert_ind,1), emitter_points(pert_ind,2), emitter_seg_array(pert_ind-1), iErr);
% [emitter_seg_array(pert_ind), iErr] = Geometry2D_CreateLine(Lorentz, emitter_points(pert_ind,1), emitter_points(pert_ind,2),emitter_points(pert_ind+1,1), emitter_points(pert_ind+1,2), emitter_seg_array(pert_ind), iErr);
% [iErr] = Object_AddSegment(Lorentz, Emitter, emitter_seg_array(pert_ind-1), iErr);
% [iErr] = Object_AddSegment(Lorentz, Emitter, emitter_seg_array(pert_ind), iErr);
% 
% pert_ind = 28;
% emitter_points(pert_ind, :) = emitter_points(pert_ind, :) + [0, 5];
% [iErr] = Object_RemoveSegment(Lorentz, Emitter, emitter_seg_array(pert_ind -1), iErr);
...
% 
% %set voltages
% [iErr] = Physics_Set2DVoltage(Lorentz, Emitter, 10, iErr);
